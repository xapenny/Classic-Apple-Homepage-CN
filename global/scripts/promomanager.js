...
}AC.PromoManager.instances.push(this);this.storageName="promomanager_"+b;this.promos=(typeof h=="string")?$$("."+h):$A(h);
...
}}.bind(this));this.selectPromo(c)},selectPromo:function(b,c){if(this.selectedPromoIndex){var a=this.promos[this.selectedPromoIndex];
if(c){a.promo.fade({duration:c})}else{a.promo.hide()}}this.selectedPromoIndex=b;
var a=this.promos[b];if(c){a.promo.appear({duration:c})}else{a.promo.show()}this.history.unshift(b);
...
setInterval(this.selectNextPromo.bind(this),this.options.interval*1000)},selectNextPromo:function(){this.rotating++;
if(this.rotating>this.managers.length-1){this.rotating=0}this.managers[this.rotating].selectNextPromo(this.options.animationDuration)
}});
